...
function buildCatalogs(config) {
  ...
  if (config?.traktAccessToken && (!config?.options || config.options.showWatchlist !== false)) {
        ...
        for (const list of config?.traktLists || []) {
                const baseId = `trakt:list:${list.username}:${list.slug}`;
                catalogs.push(
  ...
  if (config?.imdbUserId) {
        catalogs.push(
          { type: 'movie', id: 'imdb:watchlist:movies', name: 'IMDb Watchlist - Movies', extra: [{ name: 'skip' }] },
          { type: 'series', id: 'imdb:watchlist:shows', name: 'IMDb Watchlist - Series', extra: [{ name: 'skip' }] }
              );
  }
    for (const list of config?.imdbLists || []) {
          const baseId = `imdb:list:${list.id}`;
          catalogs.push(
            { type: 'movie', id: `${baseId}:movies`, name: `IMDb - ${list.name} - Movies`, extra: [{ name: 'skip' }] },
            { type: 'series', id: `${baseId}:shows`, name: `IMDb - ${list.name} - Series`, extra: [{ name: 'skip' }] }
                );
    }
